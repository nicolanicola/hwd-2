...
function starkers_setup() {
    ...
    'footer' => __( 'Footer Navigation', 'starkers' ),
    'footer2' => __( 'Secondary Footer Navigation', 'starkers' ),
	) );
}
endif;
...
function starkers_comment( $comment, $args, $depth ) {
	...
	<div class="clearfix">
		...
		<div class="author-comment-info fr">
				<p><?php printf( __( '%s', 'starkers' ), sprintf( '%s', get_comment_author_link() ) ); ?></p>
				<p><?php printf( __( '%1$s at %2$s', 'starkers' ), get_comment_date(),  get_comment_time() ); ?></p>
...
function starkers_fields($fields) {
	...
	$fields =  array(
		'author' => '<p><label for="author">' . __( 'Name' ) .( $req ? ' *' : '' ) . '</label> ' . 
		'<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
		'email'  => '<p><label for="email">' . __( 'Email' ) .  ( $req ? ' *' : '' ) . '</label> '.
