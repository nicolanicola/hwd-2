...
<?php while ( have_posts() ) : the_post(); ?>
        ...
        <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
            <header>
                <h2><a href="<?php the_permalink(); ?>"
                       title="<?php printf( esc_attr__( 'Permalink to %s', 'starkers' ), the_title_attribute( 'echo=0' ) ); ?>"
            ...
            <footer>
				<?php if ( function_exists( 'get_post_format' ) && 'gallery' == get_post_format( $post->ID ) ) : ?>
                    <a href="<?php echo get_post_format_link( 'gallery' ); ?>"
                       title="<?php esc_attr_e( 'View Galleries', 'starkers' ); ?>"><?php _e( 'More Galleries', 'starkers' ); ?></a> |
				
				<?php elseif ( in_category( _x( 'gallery', 'gallery category slug', 'starkers' ) ) ) : ?>
                    <a href="<?php echo get_term_link( _x( 'gallery', 'gallery category slug', 'starkers' ), 'category' ); ?>"
                       title="<?php esc_attr_e( 'View posts in the Gallery category', 'starkers' ); ?>"><?php _e( 'More Galleries', 'twentyten' ); ?></a> |
	...
	<?php else : ?>
        <article id="post-<?php the_ID(); ?>" <?php post_class( 'archive-post clearfix' ); ?>>
			...
			<?php if ( ! is_search() ) { ?>
<!--                <h2 class="blog-subtitle">Posted on --><?php //echo get_the_date(); ?><!--</h2>-->
                </header>
			<?php }; ?>
			
			<?php wpe_excerpt( 'wpe_excerptlength_index', 'wpe_excerptctalink' ); ?>
